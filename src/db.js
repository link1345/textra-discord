...
export async function create_db(){
  ...
  return new Promise((resolve, reject) => {
    ...
    db.serialize( ()=> {
      ...
      db.each('SELECT * FROM sqlite_master WHERE TYPE = ? AND name = ?', ["table", "user_mt_translation_mode"], function(err, row) {
        if (err){
            // エラー処理
            db.close();
            resolve();
    ...
    db.each('SELECT * FROM sqlite_master WHERE TYPE = ? AND name = ?', ["table", "guild_setting"], function(err, row) {
      if (err){
          // エラー処理
          db.close();
          resolve();
      ...
      db.each('SELECT * FROM user_mt_translation_mode WHERE user_id = ?', [user_id], function(err, row) {
        ...
        if (err){
          ...
          if (count == 0){
              // SQLの実行結果が0件の場合
              // user情報追加
              var stmt = db.prepare('INSERT INTO user_mt_translation_mode VALUES(?,?)');
        ...
        if (err){
          ...
          if (err){
              ...
              if (count == 0){
                  // SQLの実行結果が0件の場合
                  // user情報追加
                  var stmt = db.prepare('INSERT INTO guild_setting VALUES(?,?,?,?,?,?)');
    ...
    db.serialize( ()=> {
      ...
      db.each('SELECT * FROM guild_setting WHERE guild_id = ?', [guild_id], function(err, row) {
        if (err){
            ...
            // SQLの実行結果が1件以上の場合
            hit_flag = true;
            return_item = {"guild_id": row.guild_id, "mt_key": row.mt_key, "mt_secret": row.mt_secret, "mt_loginname":row.mt_loginname, "access_token":row.access_token, "access_token_time":row.access_token_time};    
